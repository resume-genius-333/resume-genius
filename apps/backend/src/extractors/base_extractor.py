...
class BaseExtractor(ABC, Generic[T]):
    ...
    async def extract_from_pdf(
        ...
        try:
            ...
            logger.error(
                f"Failed to extract from {pdf_path.name}: {str(e)}"
            )
            raise
    ...
    async def extract_from_url(
        ...
        try:
            ...
            logger.info(
                f"Successfully extracted {self.__class__.__name__} from URL"
            )
            return result
            
        except Exception as e:
            logger.error(
                f"Failed to extract from URL: {str(e)}"
            )
            raise
    ...
    def calculate_extraction_confidence(self, extracted_data: T) -> float:
        ...
        filled_fields = sum(
            1 for field_name in extracted_data.model_fields
