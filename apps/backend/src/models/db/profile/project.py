...
if TYPE_CHECKING:
    ...
    from .skill import ProfileTaskSkillMapping


from pydantic import BaseModel, PrivateAttr


class ProfileProjectSchema(BaseModel):
    id: uuid.UUID
    user_id: uuid.UUID
    project_name: str
    ...
    _orm_entity: Optional["ProfileProject"] = PrivateAttr(default=None)
...
class ProfileProject(Base):
    ...
    # Relationships
    user: Mapped["ProfileUser"] = relationship(back_populates="projects")
    tasks: Mapped[List["ProfileProjectTask"]] = relationship(
    ...
    @property
    def schema(self):
        result = ProfileProjectSchema.model_validate(self)
        result._orm_entity = self
        return result


class ProfileProjectTaskSchema(BaseModel):
    id: uuid.UUID
    ...
    created_at: datetime.datetime
    updated_at: datetime.datetime
    _orm_entity: Optional["ProfileProjectTask"] = PrivateAttr(default=None)

    class Config:
        from_attributes = True


class ProfileProjectTask(Base):
    ...
    # Relationships
    project: Mapped["ProfileProject"] = relationship(back_populates="tasks")
    skill_mappings: Mapped[List["ProfileTaskSkillMapping"]] = relationship(
    ...
    @property
    def schema(self):
        result = ProfileProjectTaskSchema.model_validate(self)
        result._orm_entity = self
        return result
